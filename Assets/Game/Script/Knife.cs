...
public class Knife : MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D coll) {
		...
		} else if ((coll.gameObject.tag == "Wood" || coll.gameObject.tag == "BossVip")  && !isHitted && !GameManager.isGameOver) {
	...
	void gameOver()
	{
		...
		if (GameManager.MaxScore < GameManager.Score) GameManager.MaxScore = GameManager.Score;
		if (GameManager.MaxStage < GameManager.Stage) GameManager.MaxStage = GameManager.Stage;
	}
