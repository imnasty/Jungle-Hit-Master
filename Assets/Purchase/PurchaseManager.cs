...
public class PurchaseManager : MonoBehaviour, IStoreListener
{
    ...
    /// </summary>
    public static event OnSuccessSubscribe OnPurchaseSubscribe;
    /// <summary>
    ...
    public void InitializePurchasing()
    {
        ...
        foreach (string s in SUB_PRODUCTS) builder.AddProduct(s, ProductType.Subscription);
    ...
    {
        currentProductIndex = index;
        BuyProductID(SUB_PRODUCTS[index]);
    }
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        if (C_PRODUCTS.Length > 0 && String.Equals(args.purchasedProduct.definition.id, C_PRODUCTS[currentProductIndex], StringComparison.Ordinal))
            ...
            OnSuccessNC(args);
        else if (SUB_PRODUCTS.Length > 0 && String.Equals(args.purchasedProduct.definition.id, SUB_PRODUCTS[currentProductIndex], StringComparison.Ordinal))
            OnSuccessSUB(args);
    ...
    public delegate void OnSuccessSubscribe(PurchaseEventArgs args);
    protected virtual void OnSuccessSUB(PurchaseEventArgs args)
    {
        if (OnPurchaseSubscribe != null) OnPurchaseSubscribe(args);
        Debug.Log(SUB_PRODUCTS[currentProductIndex] + " Buyed!");
    }
